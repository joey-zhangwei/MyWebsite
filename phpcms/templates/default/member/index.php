...
class index extends foreground {
			...
			if(!empty($_GET['verify'])) {
				...
				if($member_setting['choosemodel']) {
					...
					if(array_key_exists($modelid, $modellist)) {
						...
						$forminfos = $forminfos_arr = $member_form->get('','upload');
	...
	public function public_checkskype_ajax() {
		$skype = isset($_GET['info']['skype']) && trim($_GET['info']['skype']) ? trim($_GET['info']['skype']) : exit('0');
		if(CHARSET != 'utf-8') {
			$skype = iconv('utf-8', CHARSET,$skype);
			$skype = addslashes($skype);
		} 
		$this->jiangshi_db->tablename = "member_jiangs";
		$this->detail_db->tablenam = "member_detail";
		if($this->jiangshi_db->get_one(array("skype"=>$skype))){
			echo 99;die;
			exit('0');
		}elseif($this->detail_db->get_one(array("skype"=>$skype))){echo 66;die;
			exit('0');
		}else{
			exit('1');
		}
