...
//			}
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if($status > 0) {
					...
					if($member_setting['enablemailcheck']) {
						...
						$modstr = md5($userinfo['modelid']."model");
						$url = APP_PATH."index.php?m=member&c=index&a=register&code=$code&verify=1&modelid=".$modstr;
			...
			if(!empty($_GET['verify'])) {
				...
				$tealock = md5("12model");
				if($_GET['modelid']==$tealock){
				   include template('member', 'tealock');
				}else{
				...
				$forminfos['sex']['form']="<label class='ib' style='width:100px'><input type='radio' name='info[sex]' id='sex_男'  value='男' checked> 男</label><label class='ib' style='width:100px'><input type='radio' name='info[sex]' id='sex_女'  value='女'> 女</label>";
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($pos!=0){
			$dbs = pc_base::load_model('content_model');
			$dbs->table_name ='v9_member';		
			$datasr = $dbs->get_one(array('email'=>$username));			
			if(!$datasr)
				showmessage("您输入的邮箱错误", HTTP_REFERER);
			$username=$datasr[username];
			}
	 ...
	 * @return $status {0:已存在;1:成功}
	 */
	public function public_checkskype_ajax() {
		$skype = isset($_GET['skype']) && trim($_GET['skype']) ? trim($_GET['skype']) : exit('0');
		if(CHARSET != 'utf-8') {
			$skype = iconv('utf-8', CHARSET,$skype);
			$skype = addslashes($skype);
		} 

		$sql = "select userid from v9_member_jiangs where skype = '".$skype."'";
		$flag = $this->db->query($sql);
		$flag = $this->db->fetch_array($flag);
		$flags = $this->db->query("select userid from v9_member_detail where skype = '$skype'");
		$flags = $this->db->fetch_array($flag);
		if($flag['0']['userid']>0){
			echo $flag['0']['userid'];die;
		}elseif($flags['0']['userid']>0){
			echo '5';die;
		}else{
			echo '1';die;
		}
	...
	/**
	 * 检查手机
	 * @param string $shouji	手机
	 * @return $status {0:已存在;1:成功}
	 */
	public function public_checkmobile_ajax() {
		$shouji = isset($_GET['shouji']) && trim($_GET['shouji']) ? trim($_GET['shouji']) : exit('0');
		$shouji = addslashes($shouji);

		$sql = "select userid from v9_member_detail where shouji = '".$shouji."'";
		$flag = $this->db->query($sql);
		$flag = $this->db->fetch_array($flag);
		if($flag['0']>0){
			echo '0';die;
	 ...
	 * @return $status {0:已存在;1:成功}
	 */
	public function public_checkqq_ajax() {
		$shouji = isset($_GET['qqhao']) && trim($_GET['qqhao']) ? trim($_GET['qqhao']) : exit('0');
		$shouji = addslashes($shouji);

		$sql = "select userid from v9_member_detail where qqhao = '".$shouji."'";
		$flag = $this->db->query($sql);
		$flag = $this->db->fetch_array($flag);
		if($flag['0']>0){
			echo '0';die;
	...
	public function public_forget_password () {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$code = explode("\t", $code);
			$userinfo = $this->db->get_one(array('userid'=>$code[0]));
			if(isset($_POST['submit'])){
				$updateinfo['password'] = password($_POST['password'], $userinfo['encrypt']);
				$this->db->update($updateinfo, array('userid'=>$userinfo['userid']));
				if(pc_base::load_config('system', 'phpsso')) {
						//初始化phpsso
						$this->_init_phpsso();
						$this->client->ps_member_edit('', $email, '', $_POST['password'], $userinfo['phpssouid'], $userinfo['encrypt']);
				}
				showmessage('密码修改成功请重新登录!', 'index.php?m=member&c=index&a=login');
			}

			include template('member', 'forget_password_edit');die;
