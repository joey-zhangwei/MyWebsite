...
class content extends foreground {
	...
	$memberinfo = $this->memberinfo;
		...
		$where = "username='$_username' and siteid=$siteid and inputtime>$time_day";
		...
		$kc_where = "AND ((UNIX_TIMESTAMP(dates)>=$time_day) OR (b.dates<$time_day AND b.is_yuyue=2))";
		if($_GET[pl]==1)
		include template('member', 'xspingjia');
		elseif($_GET[jl]==1)
		include template('member', 'yuyuejl');
		else	
		include template('member', 'chuangjian');	
	}
	/**
	 * 编辑内容
	 */
