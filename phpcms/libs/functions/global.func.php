...
*/
function xiangt($key, $arr)
             {
            
			 $vs['time']=$key;
			 // echo $key;
			 // print_r($arr);
			  $as = in_array($vs['time'], $arr);
			   $arr[0]['time'];
			 // print_r($arr[0]);
...
if(!$mnow)
 ...
 echo "<table id=\"calendar\" border=\"0\" cellpadding=\"5\" cellspacing=\"0\" width=\"300\">";
 
 echo "
 <tr style=\"border-bottom-color: black; background-color: #4791da; border-top-color: black; border-right-color: black; border-left-color: black; font-size: 12px; color:#FFFFFF;\">
 <td colspan=\"4\" align=\"left\" class=\"calendartitle\">".$ynow."年".date("m",$mtime)."月</td>
 
 <td colspan=\"3\" align=\"center\">
 
  <a href=\"?month=".(intval($mnow)==12?"1":intval($mnow)+1).($ynow!=date("Y")?"&year=".intval($ynow):"")."\">  </a>
 ...
 echo "
 <tr>
 <td class=\"calendartop\">周日</td>
 <td class=\"calendartop\">周一</td>
 <td class=\"calendartop\">周二</td>
 <td class=\"calendartop\">周三</td>
 <td class=\"calendartop\">周四</td>
 <td class=\"calendartop\">周五</td>
 <td class=\"calendartop\">周六</td>
 </tr>
 ";
 ...
 for($i=0;$i<date("t",$mtime)+$f+1;$i++){
	 ...
	 if($i-$f==$td)
	  ...
	  if($setnull=='-1'){
		echo "<td".(($i-$f==intval(date("d")) && $mnow==date("m") && $ynow==date("Y"))?" id=\"calendartoday\"":"").(($i%7==0 || $i%7==6)?" class=\"calendarw\"":"")." style=\"background-color:#99FF66;\">";
	  }else{
	 ...
	 if($ss<10)
	  $d="0".$ss;
	   ...
	   // echo $i-$f;
		//else{
		 echo '<a href="index.php?m=content&c=index&a=lists&catid=15&d='.$dates.'&ds='.$ss.'&ms='.$mnow.'">';
		 echo $i-$f;
		 echo "</a>";
	 	//}
...
function datesed($datas,$setnull=''){
